...
```bash
/update-claude-md apps/bank-transactions-analyzer
/update-claude-md libs/qr-code-management/data-access
```
...
# Output format:
# M    apps/bank-transactions-analyzer/app/api/route.ts
...
async function analyzeChanges(claudeMdPath: string, files: string[]): Promise<UpdatePlan> {
  ...
  const diffs = await Promise.all(
    files.map(f => execAsync(`git diff --cached -- "${f}"`))
  );
...
if (file.path.endsWith('project.json')) {
  ...
  return {
    ...
    text: addedTargets.map(t => `- \`nx ${t.name} ${projectName}\` - ${t.description || '[TODO: Add description]'}`).join('\n')
  };
}
```
