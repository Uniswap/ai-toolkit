...
|----------|--------|---------|---------|
| [`reusable/_generate-changelog.yml`](./reusable/_generate-changelog.yml) | `before_sha`, `after_sha`, `custom_prompt_file`, `custom_prompt_text`, `max_tokens` | `changelog`, `generation_method` | Generates AI-powered changelogs from git commit ranges using Anthropic's Claude API |
| [`reusable/_notify-release.yml`](./reusable/_notify-release.yml) | `branch`, `npm_tag`, `before_sha`, `after_sha`, `changelog` | None | Sends formatted Slack notifications about package releases |
...
jobs:
  generate-changelog:
    uses: ./.github/workflows/reusable/_generate-changelog.yml
    with:
...
- **CI workflows**: Descriptive names like `pr-checks.yml`, placed in `ci/` directory
- **Release workflows**: Action-oriented names like `publish-packages.yml`, `update-production.yml`, placed in `release/` directory
- **Reusable workflows**: Prefixed with `_` (underscore) and verb-noun format like `_generate-changelog.yml`, `_notify-release.yml`, placed in `reusable/` directory

The `_` prefix convention for reusable workflows makes them:

- Easily identifiable as reusable/callable workflows
- Sorted to the top in file listings
- Clearly distinguished from triggered workflows
