...
| `toolkit_ref`                         | No       | `""` (workflow's own commit)                            | Git ref of ai-toolkit to fetch runtime assets from (scripts, prompt sections, validate-claude-auth action). Empty (default) fetches from this workflow's own commit via `job.workflow_sha`, so assets always match the running workflow and never drift. Set to `next` or a SHA only to test unreleased changes. |
...
with:
  ...
  model: 'claude-sonnet-4-6'
  # toolkit_ref omitted: assets are fetched from this workflow's own commit by default.
  # Set it to 'next' or a SHA only to test unreleased ai-toolkit changes.
secrets:
  ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
  WORKFLOW_PAT: ${{ secrets.WORKFLOW_PAT }}
```
