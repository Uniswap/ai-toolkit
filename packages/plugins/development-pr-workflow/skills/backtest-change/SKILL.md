...
description: >
  ...
  throughput target. Fires for a brand-new monitor, deploy gate, or alert just
  as it does for an edit to an existing one — a threshold invented for a gate
  that has never run needs the same historical replay, including the joint
  replay of every condition in a composite (e.g. rate AND floor together, not
  each in isolation). A proposed number is a hypothesis, not a decision: backtest
