...
By default, claude-plus uses `~/.claude/claude.json` for Claude configuration. You can customize this location using the `CLAUDE_CONFIG_DIR` environment variable:
...
When `CLAUDE_CONFIG_DIR` is set, the tool will read/write to `$CLAUDE_CONFIG_DIR/claude.json` instead of `~/.claude/claude.json`.
...
2. **Slack Token Validation** (Step 2/3)
   ...
   - Checks if the current Slack token in `$CLAUDE_CONFIG_DIR/claude.json` is valid (defaults to `~/.claude/claude.json`)
