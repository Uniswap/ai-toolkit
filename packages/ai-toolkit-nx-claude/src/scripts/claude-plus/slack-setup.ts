...
function displaySetupInstructions(): void {
  console.log('');
  displayInfo('╔══════════════════════════════════════════════════════════════════╗');
  displayInfo('║              Slack OAuth Setup Wizard                            ║');
  displayInfo('╚══════════════════════════════════════════════════════════════════╝');
  console.log('');
  console.log('To use the Slack integration, you need a refresh token and backend URL.');
  console.log('');
  console.log(colorize('📋 Step-by-Step Instructions:', 'cyan'));
  console.log('');
  console.log(colorize('  Step 1: Visit the OAuth Setup Page', 'yellow'));
  console.log(`    • Open: ${colorize(OAUTH_BACKEND_URL, 'green')}`);
  console.log('    • Click "Add to Slack" and authorize the app');
  console.log('');
  console.log(colorize('  Step 2: Copy Your Tokens', 'yellow'));
  console.log("    • After authorization, you'll see your tokens displayed");
  console.log('    • Copy the Access Token (OAuth Token) - starts with xoxp-...');
  console.log('    • Copy the Refresh Token - starts with xoxe-1-...');
  console.log('');
  console.log(colorize('  Step 3: Enter Your Configuration Below', 'yellow'));
  console.log(`    • Backend Refresh URL: ${colorize(OAUTH_BACKEND_URL, 'green')}`);
  console.log('    • Refresh Token: The xoxe-1-... token you copied');
  console.log('');
}
...
async function promptForCredentials(rl: readline.Interface): Promise<SlackCredentials | null> {
  ...
  console.log(colorize(`(Slack token will be saved to ${claudeConfigDir}/claude.json)`, 'gray'));
  console.log('');
  ...
  console.log(colorize(`  → Using: ${refreshUrl}`, 'green'));
  console.log('');
