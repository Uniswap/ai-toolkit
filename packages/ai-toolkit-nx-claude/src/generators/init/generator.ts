...
// Import available commands, agents, and skills from content packages
...
import { skills as agnosticSkills } from '@ai-toolkit/claude-skills';
...
// Recommended default skills for most users
...
function checkExistingSkills(targetDir: string, items: string[]): Set<string> {
  const existing = new Set<string>();
  const dir = path.join(targetDir, 'skills');
...
export async function initGenerator(tree: Tree, options: InitGeneratorSchema) {
  ...
  const globalExistingSkills = checkExistingSkills(globalDir, Object.keys(agnosticSkills));
  ...
  const localExistingSkills = checkExistingSkills(localDir, Object.keys(agnosticSkills));
  ...
  // Install selected skills (skills are directories, not single files)
  ...
  for (const skillName of skillsToInstall) {
    ...
    // First check for bundled content (when running as standalone package)
    const bundledSkillsDir = path.join(__dirname, '..', '..', 'content', 'skills');
    if (fs.existsSync(bundledSkillsDir)) {
      const potentialPath = path.join(bundledSkillsDir, skillName);
    ...
    // Fall back to workspace lookup if not found in bundled content
    if (!sourceDir) {
      const skillsBaseDir = path.join(workspaceRoot, 'packages/claude-skills/src/skills');
      if (fs.existsSync(skillsBaseDir)) {
        const potentialPath = path.join(skillsBaseDir, skillName);
        if (fs.existsSync(potentialPath) && fs.statSync(potentialPath).isDirectory()) {
          sourceDir = potentialPath;
        }
      }
    ...
    const relativeDestDir = path.join(relativeSkillsDir, skillName);
    ...
    try {
      if (sourceDir && fs.existsSync(sourceDir)) {
        ...
        for (const file of skillFiles) {
          ...
          const destFilePath = path.join(relativeDestDir, file);

          // Only copy files, not directories
          if (fs.statSync(sourceFilePath).isFile()) {
            const content = fs.readFileSync(sourceFilePath, 'utf-8');
            if (!isDryRun) {
              tree.write(destFilePath, content);
            }
            installedFiles.push(path.relative(targetDir, path.join(destDir, file)));
          }
        }
